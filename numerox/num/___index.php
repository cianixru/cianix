...
<?php
  ...
  if(!empty($name1)) # if(isset($_GET['submit']))
  {
  $name1 = $HTTP_GET_VARS["name1"];
$name2 = $HTTP_GET_VARS["name2"];
$name3 = $HTTP_GET_VARS["name3"];
$bdate1 = $HTTP_GET_VARS["bdate1"];
$bdate2 = $HTTP_GET_VARS["bdate2"];
$bdate3 = $HTTP_GET_VARS["bdate3"];
$myear = $HTTP_GET_VARS["myear"];
...
$jp = $nums->sum_nums($nums->sum_nums($bdate1,1)+$nums->sum_nums($bdate2,1)+$nums->sum_nums($bdate3,1),0);
echo '<h3>Жизненный путь: '.$jp.'</h3>';

$chs = $nums->sum_nums($nums->sum_nums($nums->cnv2nums($name1, 2, 0),1)+$nums->sum_nums($nums->cnv2nums($name2, 2, 0),1)+$nums->sum_nums($nums->cnv2nums($name3, 2, 0),1),0);
echo '<h3>Число судьбы(Экспрессия): '.$chs.'</h3>';

$dp = $nums->sum_nums($nums->sum_nums($nums->vowels($name1, 0,0),0)+$nums->sum_nums($nums->vowels($name2, 0,0),0)+$nums->sum_nums($nums->vowels($name3, 0,0),0),0);
echo '<h3>Душевное побуждение: '.$dp.'</h3>';

echo '<h3>Число рождения: '.$bdate1;

$chl = $nums->sum_nums($nums->sum_nums($nums->vowels($name1, 1,0),0)+$nums->sum_nums($nums->vowels($name2, 1,0),0)+$nums->sum_nums($nums->vowels($name3, 1, 0),0),0);
echo '<h3>Число личности: '.$chl.'</h3>';

$chr = $nums->sum_nums($jp+$chs,0);
echo '<h3>Число Реализации: '.$chr.'</h3>';

$chrzm = $nums->sum_nums($nums->sum_nums($nums->cnv2nums($name1, 2, 0),0)+$nums->sum_nums($bdate1,0),1);
...
echo '<br>Кармические уроки: '; $karma_lesns = $nums->spectr_name($name1,$name2,$name3,1);
...
echo '<br>Число роста: '; echo $ch_rosta;
